...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { userEmail, affiliateID } = requestData;
    ...
    if (affiliateID && (typeof affiliateID !== 'string' || affiliateID.trim().length === 0)) {
      return {
        statusCode: 400,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: "Invalid affiliate ID format" })
      };
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${siteUrl}/app/receipt.html?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${siteUrl}/product.html?canceled=true`,
      metadata: {
    ...
    if (dbError) {
      ...
      console.log(`Transaction logged in database for session: ${session.id}`);
    }

    const processingTime = Date.now() - startTime;
    console.log(`Checkout session created successfully in ${processingTime}ms`);
