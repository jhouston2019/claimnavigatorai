...
exports.handler = async (event, context) => {
  // Only allow POST (for manual triggers) or scheduled events
  ...
  try {
    ...
    const { data: leadExchanges, error: leadsError } = await supabase
      .from('lead_exchange')
      .select(`
        *,
        leads:original_lead_id (
          insured_name,
          phone,
          email,
          date_of_loss,
    ...
    if (!leadExchanges || leadExchanges.length === 0) {
    ...
    const leadsByState = {};
    leadExchanges.forEach(leadExchange => {
      const lead = leadExchange.leads;
      if (!lead) return;
      
      const state = lead.loss_location && lead.loss_location.includes(',') 
      ...
      leadsByState[state].push({
        ...lead,
        price: leadExchange.price,
        lead_exchange_id: leadExchange.id
      });
    });
    ...
    return {
      ...
      body: JSON.stringify({ 
        ...
        leads_count: leadExchanges.length,
